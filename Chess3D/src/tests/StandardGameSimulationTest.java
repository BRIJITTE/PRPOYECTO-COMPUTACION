...
/**
* Simula un juego de ajedrez (corto) que resulta en jaque mate negro.
  *
 */
public class StandardGameSimulationTest {
	
	//Método para realizar un movimiento simulado
	private static void doMove(int xs, int ys, int xe, int ye, GameLoop game, GLCanvas canvas, int seconds) {
		try {
			Thread.sleep(1000 * seconds); // tiempo de espera//
	...
	public static void main(String[] args) {
		...
		doMove(3, 1, 3, 3, game, glcanvas, SECONDS); 
		System.out.println(game.getBoard().getAllMoves(game.getPlayer2()).size() + " moves left for black");
		doMove(3, 6, 3, 4, game, glcanvas, SECONDS); 
		System.out.println(game.getBoard().getAllMoves(game.getPlayer1()).size() + " moves left for white");
		
		doMove(4, 0, 2, 2, game, glcanvas, SECONDS); 
		System.out.println(game.getBoard().getAllMoves(game.getPlayer2()).size() + " moves left for black");
		doMove(7, 6, 7, 5, game, glcanvas, SECONDS); 
		System.out.println(game.getBoard().getAllMoves(game.getPlayer1()).size() + " moves left for white");
		
		doMove(2, 0, 5, 3, game, glcanvas, SECONDS); 
		System.out.println(game.getBoard().getAllMoves(game.getPlayer2()).size() + " moves left for black");
		doMove(7, 5, 7, 4, game, glcanvas, SECONDS); 
		System.out.println(game.getBoard().getAllMoves(game.getPlayer1()).size() + " moves left for white");
		
		doMove(2, 2, 2, 6, game, glcanvas, SECONDS); 
		System.out.println(game.getBoard().getAllMoves(game.getPlayer2()).size() + " moves left for black");
